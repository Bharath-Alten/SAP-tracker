...
function findSheetRows(payload, names) {
...
export async function runMockPlaywrightAutomation(payload, artifactDir, onProgress) {
    const emit = (event) => {
        if (onProgress) {
            onProgress({
                ...event,
    ...
    try {
        ...
        if (artifactDir) {
            try {
                ...
                emit({ type: 'error', step: 'Artifact error', message: `Failed to save screenshot: ${e.message}` });
            }
        }
