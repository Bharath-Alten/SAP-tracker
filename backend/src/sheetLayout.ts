...
import type { CellRef, LayoutItem, SheetLayout } from './types.js';

// Turns a document-style sheet (e.g. "CP Front Page") into titled sections of fields.
// Section headings are recognised by their dark fill (the blue boxes in the workbook).
// Every CellRef points back into the normalized sheet rows, so edits made through the
// form update the same data that is saved and passed to the automation.

type LayoutCell = { r: number; col: number; text: string; dark: boolean; ref: CellRef };
type LayoutRow = { r: number; index: number; cells: LayoutCell[] };
type Draft = { title: string; headingRow: number; subLabels: LayoutCell[]; headerRows: LayoutRow[]; rows: LayoutRow[] };
...
function mapRowsToParsedIndex(worksheet: XLSX.WorkSheet, range: XLSX.Range, parsedRows: unknown[][]) {
  const allRows = XLSX.utils.sheet_to_json(worksheet, { header: 1, raw: false, blankrows: true, defval: '' }) as unknown[][];
...
export function buildSheetLayout(worksheet: XLSX.WorkSheet, parsedRows: unknown[][]): SheetLayout | null {
  if (!worksheet['!ref']) return null;
  const range = XLSX.utils.decode_range(worksheet['!ref']);
  const indexByRow = mapRowsToParsedIndex(worksheet, range, parsedRows);
  if (!indexByRow) return null;

  const merges = worksheet['!merges'] ?? [];
  const mergeAt = (r: number, c: number) => merges.find((m) => r >= m.s.r && r <= m.e.r && c >= m.s.c && c <= m.e.c);
  const spanEnd = (r: number, c: number) => mergeAt(r, c)?.e.c ?? c;
  const keyFor = (c: number) => `C${c - range.s.c + 1}`;

  const rows: LayoutRow[] = [];
  for (const [r, index] of indexByRow) {
    const cells: LayoutCell[] = [];
    for (let col = range.s.c; col <= range.e.c; col += 1) {
      const cell = worksheet[XLSX.utils.encode_cell({ r, c: col })] as XLSX.CellObject | undefined;
      const text = cellText(cell);
      if (text) cells.push({ r, col, text, dark: isDarkFill(cell), ref: { row: index, key: keyFor(col) } });
    }
    if (cells.length) rows.push({ r, index, cells });
  }
  if (!rows.some((row) => row.cells.some((cell) => cell.dark))) return null;

  // 1. Split rows into sections at dark heading rows.
  const layout: SheetLayout = { sections: [] };
  const drafts: Draft[] = [];
  rows.forEach((row, position) => {
    if (!row.cells.every((cell) => cell.dark)) {
      if (!drafts.length) drafts.push({ title: 'General', headingRow: row.r, subLabels: [], headerRows: [], rows: [] });
  ...
  const codeByCol = new Map<number, string>();

  // 2. Turn each section's rows into fields, a choice list or a table.
  for (const draft of drafts) {
    const used = new Set<LayoutCell>();
    const items: Array<{ order: number; item: LayoutItem }> = [];
    const add = (r: number, col: number, item: LayoutItem) => items.push({ order: r * 1000 + col, item });
    const cellAt = (row: LayoutRow | undefined, col: number) => row?.cells.find((cell) => cell.col === col);
    const subLabelFor = (col: number) => draft.subLabels.find((label) => col >= label.col && col <= spanEnd(draft.headingRow, label.col))?.text;
    ...
    draft.rows.forEach((row, i) => {
      ...
      if (!next || !arrow || !nextArrow || used.has(arrow)) return;
      used.add(arrow);
      used.add(nextArrow);
    ...
    if (!draft.headerRows.length) {
      for (const row of draft.rows) {
        ...
        if (!label || label.dark || !label.text.endsWith(':')) continue;
    ...
    draft.rows.forEach((row, i) => {
      ...
      for (const label of row.cells) {
        if (!label.dark || used.has(label) || label.text.endsWith('->')) continue;
        const below = next && next.r === row.r + 1 ? cellAt(next, label.col) : undefined;
        if (!below || below.dark || used.has(below)) continue;
        used.add(label);
        used.add(below);
    ...
    if (draft.headerRows.length) {
      ...
      const span = (cell: LayoutCell) => [cell.col, spanEnd(cell.r, cell.col)] as const;
      const columns = new Map<number, string>();
      const leafSpans: Array<readonly [number, number]> = [];
      ...
      const covered = (col: number) => leafSpans.some(([start, end]) => col >= start && col <= end);
      const dataRows = draft.rows.filter((row) => row.cells.some((cell) => !used.has(cell)));
      for (const cell of dataRows.flatMap((row) => row.cells)) {
        if (!columns.has(cell.col) && !covered(cell.col)) columns.set(cell.col, codeByCol.get(cell.col) ?? `Column ${XLSX.utils.encode_col(cell.col)}`);
        used.add(cell);
      }
      if (dataRows.length) {
